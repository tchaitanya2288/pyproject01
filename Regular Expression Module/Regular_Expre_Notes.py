...
any
confusion
while dealing with regular expressions,
we
would
use
Raw
...
"word"
character: a
letter or digit or underbar[a - zA - Z0 - 9
_].
\W - - matches
any
non - word
...
function
searches
for first occurrence of RE pattern within string with optional flags.
...
Except
for control characters, (+ ?.* ^ $ ()[] {} |  \), all characters match themselves.
...
\brub\B    \B is nonword
boundary: match
"rub" in "rube" and "ruby"
but
not alone
Python(?=!)    Match
"Python",
if followed by an exclamation point.
...
omment)	Matches "R". All the rest is a comment
R(?i)uby    Case-insensitive
while matching "uby"
R(?i: uby)    Same as above
rub(?:y | le))    Group
only
